...
<template>
  ...
  <div v-if="showUploadModal" class="overlay" @click="showUploadModal = false">
    <div class="modal3" @click.stop>
      <div class="topUpload">
        ...
        <div class="upload">
          <button class="loginBtn" onclick="document.getElementById('getFile').click()" v-on:click="submitFile">Select
            file</button>
  ...
  <body>
    ...
    <div class="Rfq">
      ...
      <div class="topBotton">
        <button class="topButton" v-on:click="showUploadModal = true"
          v-if="stores.signedInUserRole == 'Finance Admin'">Upload Contract Note</button>
          <v-btn :loading="loading"  height="48" rounded="xl" v-on:click.prevent="sendEmail" size="small" variant="flat">Notify Winner for Signature</v-btn>
        <!-- <button class="topButton" v-on:click="showRatingModal = true">Rate Bidders Performance</button> -->
        <b>{{ message }}</b>
      </div>

      <div class="mainPage">
        <div class="contractPreview"><iframe :src=stores.testDocument3 height="100%" width="100%"
            title="Purchase Contract Preview"></iframe></div>
        <div class="comments">
          <div>
            <b>Comment (Optional)</b>
            <textarea class="textarea" name="textarea" rows="17" cols="55" v-model.trim="newComment"
          ...
          <div class="uploadedfiles">
            ...
            <p style="cursor: pointer;">Preview</p>
            <p>5.7MB</p>
          </div>
...
export default {
  ...
  created() {
    this.rfqNo = 'RFQ00048'
    this.bidderName = 'Solar Consults Nigeria Limited'
    this.toWhom = 'samuel.odunuyi@codeware.com.ng'
    this.purchaseContractNo = 'PO10085'
  },
  ...
  methods: {
    sendEmail() {
      ...
      try {
        emailjs.send("service_beqemfz", "rfq_notification", {
          bidderName: this.bidderName,
          rfqNo: this.rfqNo,
          toWhom: this.toWhom,
          purchaseContractNo: this.purchaseContractNo,
        }, 'WvCTIZEVzCj8xv4Oy');
        setTimeout(async () => {
        this.loading = false
        this.message='sent'
